...
import {
	Flashcard,
	FlashCardToCreateFlashCard,
} from "@/app/interface/flashcard/flashcard";
...
export default function PopupAddFlashcard() {
	...
	const close = () => {
		...
		setSearchValue("");
		setFtagValue("");
		onChangeIsOpenAddFlashcard(false);
	};
	...
	const addNewTag = (e: FormEvent) => {
		...
		const createFullFlashCard: CreateFullFlashCard = {
			file: images,
			data: FlashCardToCreateFlashCard(flashcard),
		};
		addFlashcard(createFullFlashCard).then((success) => {
			if (success) {
				close();
			}
		});
	...
	const onChangeWord = (word: string) => {
		setFlashcard({ ...flashcard, word });
	};

	const onChangeDefinition = (definition: string) => {
		setFlashcard({ ...flashcard, definition });
	};
	...
	const onDeleteTag = (tag: FTag) => {
		...
		onSetConfirmation({
			message: `Do you want to delete "${tag.value}"?`,
			onConfirm: del,
			type: "delete",
		});
	};
