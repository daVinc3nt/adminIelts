...
export class TestOperation {
	...
	async countRecordByTestId(testId: UUID, token: string) {
		try {
			const response: AxiosResponse = await axios.get(
				`${this.baseUrl}/records/count/${testId}`,
				{
					withCredentials: true,
					validateStatus: (status) => status >= 200 && status <= 500,
					headers: {
...
export class RecordOperation {
	...
	async getWritingAnswer(writingAnswerId: UUID, token: String) {
		try {
			const response: AxiosResponse = await axios.get(
				`${this.baseUrl}/writing/answer/${writingAnswerId}`,
				{
					withCredentials: true,
					validateStatus: (status) => status >= 200 && status <= 500,
					headers: {
	...
	async createWritingRemark(
		writingAnswerId: UUID,
		payload: RemarkWriting,
		token: String
	) {
		try {
			const response: AxiosResponse = await axios.post(
				`${this.baseUrl}/writing/remark/${writingAnswerId}`,
				payload,
				{
					withCredentials: true,
					validateStatus: (status) => status >= 200 && status <= 500,
...
export class PracticeOperation {
	...
	async search(tagId: UUID, page: string, size: string, token: string) {
		try {
			const response: AxiosResponse = await axios.get(
				`${this.baseUrl}/search?tagId=${tagId}&page=${page}&size=${size}`,
				{
					withCredentials: true,
					validateStatus: (status) => status >= 200 && status <= 500,
					headers: {
