...
export default function Pie({duration}) {
  ...
  useEffect(() => {
    const intervalId = setInterval(() => {
      if (timeRemaining > 0) {
        if (window)
          {
            localStorage.setItem("timer", (timeRemaining - 1).toString())
  ...
  return (
    <>
      <div
        onKeyDown={()=>{window.onbeforeunload = () => {
          return true;
        }}}
        className={`relative size-28 lg:size-32  ${timeRemaining/duration<0.1? "animate-tada": ""}`}>
        <svg className="size-full" width="36" height="36" viewBox="0 0 36 36" xmlns="http://www.w3.org/2000/svg">
          ...
          <g className="origin-center -rotate-90 transform duration-200 ease-linear transition-all">
            ...
            strokeDashoffset={timeRemaining/duration *100}>
            </circle>
          </g>
        </svg>
        <div className="absolute top-1/2 start-1/2 transform -translate-y-1/2 -translate-x-1/2">
          <span className="text-center text-lg lg:text-2xl font-bold text-gray-800">{formatTime()}</span>
        </div>
      </div>
    </>
  );
